from flask import Flask, request, jsonify
import requests

app = Flask(__name__)

@app.route('/ask', methods=['GET'])
def ask_jarvis():
    user_query = request.args.get('text', '')
    if not user_query:
        return jsonify({"reply": "No instructions received, sir."})
        
    try:
        system_instruction = "You are Jarvis, a powerful personal AI assistant. Sufiyan is your creator. Keep responses concise, direct, helpful, and address him as sir."
        
        url = "https://pollinations.ai"
        payload = {
            "messages": [
                {"role": "system", "content": system_instruction},
                {"role": "user", "content": user_query}
            ],
            "model": "openai"
        }
        
        response = requests.post(url, json=payload, timeout=25)
        if response.status_code == 200:
            return jsonify({"reply": response.text.strip()})
            
    except Exception as e:
        return jsonify({"reply": f"Cognitive relay error: {str(e)}, sir."})
        
    return jsonify({"reply": "My remote cognitive servers are currently re-calibrating, sir."})

if __name__ == '__main__':
    app.run(host='0.0.0.0', port=5000)
  
